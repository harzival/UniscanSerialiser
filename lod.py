...
class Lod(object):
    def __init__(
        self, lod_dir_name=None, geometric_error=None, obj_root_path=None
    ):
    ...
    def get_lod_list(root_path, geometric_error_list):
        ...
        for i, lod_dir in enumerate(scandir(root_path)):
            lod_list.append(
                Lod(lod_dir.name, geometric_error_list[i], root_path)
            )
        return lod_list

    def populate_obj_list(self):
        self.obj_list = Obj.get_obj_list_from_lod_path(
            self.obj_root_path, self.lod_dir_name
        )
        return self
